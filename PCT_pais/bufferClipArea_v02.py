...
for i in teste:
    ...
    ofile2=unicode(""+os.path.splitext(mosaic_reclass)[0]+'_'+i+'_shp_buffer_200m.shp')
    ofile3=unicode(""+os.path.splitext(mosaic_reclass)[0]+'_'+i+'_shp_buffer_200m_Temp.shp')
    
    #arcpy.Clip_analysis(mosaic_reclass, i+'_shp_buffer_200m', ofile2, "")
    arcpy.Clip_analysis("mosaic_reclass",i+'_shp_buffer_200m',ofile2,"")
    arcpy.CalculateAreas_stats(ofile2, ofile3)
    arcpy.AddField_management(ofile3, 'areaHA', "DOUBLE", 20, 20)
    arcpy.AddField_management(ofile3, 'PCT_veg', "DOUBLE", 20, 20)
    
    arcpy.CalculateField_management(ofile3,"areaHA",'!F_AREA!/10000',"PYTHON")
    field = "areaHA"
    with arcpy.SearchCursor(ofile3):
        for row in cursor:
            summed_total = 0
            
            expressao='!areaHA!/'+summed_total+'*100'
            arcpy.CalculateField_management(ofile3,"PCT_veg",'!areaHA!/10000',"PYTHON")
